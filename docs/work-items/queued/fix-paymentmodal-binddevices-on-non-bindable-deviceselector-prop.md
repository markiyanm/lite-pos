...
description: PaymentModal uses bind:devices on DeviceSelector, but devices is not declared as bindable in DeviceSelector props. The device list update binding is silently non-functional. Declare devices as bindable or use a callback pattern.
status: queued
pipeline: bugfix
priority: P1
